...
Finds the things in text and files that you can't see, and takes them out.

Text you paste can carry invisible Unicode that survives a copy and paste,
bidirectional controls that make it render differently from how it's stored, and
homoglyphs standing in for ASCII letters. Files carry metadata blocks recording
who made them, with what, and when. Unmark shows you all of it first, then
strips it.

## Where it runs

Every byte is parsed in the tab. There are no network requests for your content,
no analytics, no third-party scripts and no accounts. `localStorage` holds a
theme preference and the fact you accepted the terms. Close or refresh and
nothing is left.
...
2. **Arabic and Indic orthography.** ZWNJ and ZWJ next to those scripts are required spelling, not fingerprints, so Persian `می‌خواهم` comes through intact.
...
- Hand-written PNG / JPEG / WebP container parsers, so no image library and no re-encoding
